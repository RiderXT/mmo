...
import panelFill from "../../assets/frames/panel-fill.png";
 ...
 *
 * `cornerStyle="ornate"` swaps in OrnateCorners (real engraved-gold corner art), OrnateEdges
 * (matching straight bands connecting them into an actual frame — see OrnateEdges.tsx for why a
 * plain CSS `border` line never worked here, tried twice) and a tiled stone-texture fill instead
 * of the flat `bg-panel` color — opt-in per surface (currently just the expedition map) rather
 * than the app-wide default. `panel-fill.png` had a real radial vignette in the source photo
 * (bright center, dark corners) that would have tiled into a visible grid of "hot spots" —
 * corrected with a flat-field division (divide by a heavily blurred copy of itself) before crop,
 * not just cropped as-is. */
export function PanelFrame({
  ...
  return (
    <div
      className={`relative ${ornate ? "" : "bg-panel border"} ${
        secondary
          ? `${ornate ? "" : "border-gold/25"} shadow-[0_4px_14px_rgba(0,0,0,0.35)]`
          : `${ornate ? "" : "border-gold/40"} shadow-[0_8px_24px_rgba(0,0,0,0.5)]`
      } ${className}`}
      style={ornate ? { backgroundImage: `url(${panelFill})`, backgroundRepeat: "repeat" } : undefined}
    >
      {ornate ? (
        <>
